        ...
        for batch in range(self.up_error_T.shape[0]):
            for kernel in range(self.up_error_T.shape[1]):
                ...
                for ch in range(self.input_tensor.shape[1]):  
                    return_tensor[batch, ch, :] += convolve2d(self.up_error_T[batch, kernel, :], self.weights[kernel, ch, :], 'same')  
            ...
            for kernel in range(self.num_kernels):
                for c in range(self.input_tensor.shape[1]):
                    ...
                    self.gradient_weights[kernel, c, :] += correlate2d(self.de_padded[batch, c, :], self.up_error_T[batch, kernel, :], 'valid') 
